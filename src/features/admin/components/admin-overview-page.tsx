  ...
  return (
    <div className="space-y-7">
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <Metric
          detail="Registered storefront users"
          label="Customers"
  ...
  const rows = [
    {
      ...
      label: "New customers",
      values: [
        activity.customerRegistrations.today,
        activity.customerRegistrations.week,
        activity.customerRegistrations.month,
      ],
    },
    {
      color: "bg-emerald-600",
      label: "Orders received",
      values: [activity.orders.today, activity.orders.week, activity.orders.month],
    },
  ];
  ...
  return (
    <article className="p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-xl font-extrabold tracking-tight text-[#102846]">
            Customer and order activity
          </h3>
          <p className="mt-1 text-sm text-slate-500">
            Live totals from the customer and order records in SQLite.
          </p>
        </div>
      ...
      <div
        ...
        role="img"
        aria-label="Comparison of customer registrations and orders for today, the last seven days, and this month"
      >
        {rows.map((row) => (
          <div key={row.label}>
            ...
            <div className="space-y-3">
              {row.values.map((value, index) => (
                <div
                  ...
                  key={periods[index]}
                >
                  <span className="text-xs font-semibold text-slate-500">{periods[index]}</span>
                  <div className="h-3 overflow-hidden rounded-full bg-slate-100">
                    <div
                      className={`h-full min-w-1 rounded-full ${row.color}`}
                      style={{ width: `${Math.max((value / max) * 100, value ? 4 : 0)}%` }}
                    />
                  </div>
                  <span className="w-8 text-right text-sm font-extrabold tabular-nums text-[#102846]">
                    {value}
                  </span>
                </div>
              ))}
...
function RevenueSummary({
  ...
  const periods = [
    { label: "Today", value: values.today },
    { label: "Last 7 days", value: values.week },
    { label: "This month", value: values.month },
  ];
  const max = Math.max(...periods.map((period) => period.value), 1);
  return (
    <article className="border-t border-slate-200 bg-slate-50/70 p-6 sm:p-8 xl:border-t-0 xl:border-l">
      <p className="text-sm font-bold text-[#102846]">Order value</p>
      <p className="mt-2 text-3xl font-extrabold tracking-tight text-[#102846]">
        {formatAedFromCents(values.month)}
      </p>
      <p className="mt-1 text-sm text-slate-500">Total received this month</p>
      <div
        className="mt-8 flex h-36 items-end gap-4"
        aria-label="Order value comparison by period"
        role="img"
      >
        {periods.map((period) => (
          <div className="flex flex-1 flex-col items-center gap-2" key={period.label}>
            <span className="text-center text-xs font-bold tabular-nums text-slate-700">
              {formatAedFromCents(period.value)}
            </span>
            <div className="flex h-20 w-full items-end rounded-t-xl bg-slate-200 p-1">
              <div
                className="w-full rounded-lg bg-gradient-to-t from-[#f05a28] to-[#fb923c]"
                style={{ height: `${Math.max((period.value / max) * 100, period.value ? 8 : 0)}%` }}
              />
            </div>
            <span className="text-center text-[11px] font-bold text-slate-500">{period.label}</span>
          </div>
        ))}
      </div>
      <p className="mt-7 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-900">
        Values are order totals, not forecasts. They update as orders are recorded.
      </p>
    </article>
  );
}
