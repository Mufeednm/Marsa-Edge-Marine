...
export function AdminOrdersPage({
  ...
  const selectedOrder = orderDetails.find((order) => order.id === selectedOrderId) ?? null;

  return (
    <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 p-5">
        <h2 className="text-lg font-bold text-[#102846]">Customer order requests</h2>
        <p className="mt-1 text-sm text-slate-500">
          Review the payment and fulfilment statuses before taking an order action.
        </p>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {orders.map((order) => (
              <tr className="hover:bg-slate-50" key={order.id}>
                ...
                <td className="px-5 py-4 text-slate-600">{order.shippingZone}</td>
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    {order.status === "new" ? <OrderActions id={order.id} /> : null}
                  </div>
                </td>
              </tr>
            ))}
      ...
      {selectedOrder ? (
        <OrderDetailModal close={() => setSelectedOrderId(null)} order={selectedOrder} />
      ) : null}
    </section>
  );
}
