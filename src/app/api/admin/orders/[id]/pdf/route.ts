...
export async function GET(
  ...
  document.moveDown(1.5).fillColor("#102846").fontSize(16).text(customerOrderReference);
  document
    .fillColor("#52606d")
    .fontSize(10)
    .text(
      `Placed ${new Intl.DateTimeFormat("en-AE", { dateStyle: "medium", timeStyle: "short" }).format(new Date(order.orderDate))}`,
    );
  ...
  document
    .fillColor("#334155")
    .fontSize(10)
    .text(safeText(order.deliveryAddress || order.shippingZone));
  ...
  document
    ...
    .fillColor("#52606d")
    .text("PRODUCT", 48)
    .text("QTY", 390, document.y - 11)
    .text("TOTAL", 460, document.y - 11);
  document
  ...
  for (const item of order.items) {
    document
      ...
      .fillColor("#102846")
      .fontSize(10)
      .text(safeText(item.name), 48, document.y, { width: 320 });
    const rowY = document.y - 12;
    document.fillColor("#334155").text(String(item.quantity), 390, rowY);
    document
      .fillColor("#102846")
      .text(formatAedFromCents(item.lineTotalAedCents), 460, rowY, { align: "right", width: 87 });
  }
  document
    .moveDown(1.5)
    .moveTo(330, document.y)
  ...
  document
    ...
    .fillColor("#102846")
    .fontSize(12)
    .text("Order total", 330)
    .text(formatAedFromCents(order.totalAedCents), 460, document.y - 14, {
      align: "right",
      width: 87,
    });
  document
    ...
    .fontSize(9)
    .text(
      `Payment: ${safeText(order.paymentMethod === "cod" ? "Cash on Delivery" : order.paymentMethod)}    Status: ${safeText(order.status)}`,
    );
  document.end();

  return new Response(Uint8Array.from(await pdf), {
    headers: {
      "Content-Disposition": `attachment; filename=\"marsa-edge-order-${order.customerOrderNumber}.pdf\"`,
